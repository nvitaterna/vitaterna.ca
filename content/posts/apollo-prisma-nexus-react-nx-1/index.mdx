...
In this article, you will build a simple GraphQL API that tracks todo items. You'll create this using Apollo-Express and it will be consumed by a React frontend. This will focus on getting a boilerplate setup in a monorepo making use of Nx's toolset.
...
```bash noLineNumbers
yarn create nx-workspace nstock --package-manager yarn
```
...
Nx has another option for running commands for <Link to="https://nx.dev/latest/angular/cli/affected">affected</Link> projects. This requires your dependency tree to be setup correctly - which is usually taken care of by imports. You can take a look at the dependency graph by running the following command:
...
```json:title=nx.json noLineNumbers {5,6}
{
  "tasksRunnerOptions": {
    "default": {
      "options": {
...
Once again, you'll see that the tasks ran in the correct order. Make a commit via git to save your changes and continue with the next step.
...
Go ahead and run the following. By default Nx calculates affected projects based on differences from the master branch. You can use the `uncommitted` argument to base it of what hasn't been committed instead:
